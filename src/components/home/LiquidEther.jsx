...
`;
    ...
    class Simulation {
      ...
      getFloatType() {
        // Universal compatibility: HalfFloatType is standardized for rendering in WebGL 2 across platforms.
    ...
    const ro = new ResizeObserver(() => {
      ...
      resizeRafRef.current = requestAnimationFrame(() => {
        ...
        // If resized to a visible size and is intersecting, make sure the simulation is running
        if (isVisibleRef.current && !document.hidden && !webglRef.current.running) {
          webglRef.current.start();
        }
      });
    });
    ...
    colors ? colors.join(',') : '' // Mount effect only recreates WebGL context if color palette structure changes
  ]);
